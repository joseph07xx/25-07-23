(function() {
  ...
  let indicesAleatorios = []; // Control para no repetir fotos
  ...
  // Función para generar una lista barajada de índices sin repetición
  function generarIndicesBarajados(length) {
  ...
  function iniciarProyeccion() {
    ...
    // Inicializamos la lista barajada de índices
    indicesAleatorios = generarIndicesBarajados(frames.length);
    let pasoIndex = 0;

    currentIndex = indicesAleatorios[pasoIndex];
    ...
    intervaloCambio = setInterval(() => {
      ...
      pasoIndex++;
      // Si ya pasamos por todas las fotos, volvemos a barajar para que el ciclo continúe sin aburrir
      if (pasoIndex >= indicesAleatorios.length) {
        indicesAleatorios = generarIndicesBarajados(frames.length);
        pasoIndex = 0;
      }
